      ...
      {open && (
        <section className="chatbot-panel" aria-label="AssistDesk help chatbot">
          <div className="chatbot-header">
            <div>
              <strong>AssistDesk Help</strong>
              <span>FAQ and request support</span>
            </div>
            <button type="button" className="chatbot-close" aria-label="Close chatbot" onClick={() => setOpen(false)}>×</button>
          </div>
          <div className="chatbot-body">
            {history.length === 0 && <p className="chatbot-welcome">Ask a question, choose an FAQ, or type “submit request: ...” to create a ticket.</p>}
            ...
            {sending && <div className="chat-message assistant"><span>Checking the helpdesk knowledge base...</span></div>}
            {showFaqs && (
              <div className="chatbot-faqs">
                <div className="chatbot-faq-title">Frequently asked questions</div>
          ...
          <form className="chatbot-input" onSubmit={sendMessage}>
            <input value={message} onChange={(event) => setMessage(event.target.value)} placeholder="Ask AssistDesk..." aria-label="Ask AssistDesk" />
            <button type="submit" aria-label="Send message" disabled={sending}>Send</button>
          </form>
          <button type="button" className="chatbot-faq-toggle" onClick={() => setShowFaqs((value) => !value)}>{showFaqs ? 'Hide FAQ list' : 'Show FAQ list'}</button>
        </section>
      )}
