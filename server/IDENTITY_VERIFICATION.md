...
AssistDesk accepts the verification only when `match` is `true` and `confidence` is at least `0.8`. If the provider is missing, unavailable, or returns a mismatch, the account does not become active and no login token is issued. Configure the URL over HTTPS in the server environment before enabling public registration.
